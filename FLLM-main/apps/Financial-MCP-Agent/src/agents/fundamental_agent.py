...
from src.skills.registry import get_agent_tool_bundle, describe_agent_tool_bundle
...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Available raw MCP tools: {tool_names}")

            tool_bundle = get_agent_tool_bundle("fundamental", mcp_tools)
            bundle_info = describe_agent_tool_bundle("fundamental", mcp_tools)
            logger.info(f"Tool bundle for fundamental: {bundle_info}")

            # 3. 创建ReAct Agent - 优先传入skills，再保留必要底层工具作为回退
            logger.info(
                f"{WAIT_ICON} FundamentalAgent: Creating ReAct agent...")
            agent = create_react_agent(llm, tool_bundle)
...
请优先使用高层技能 company_profile_skill 和 fundamental_analysis_skill 做首轮数据采集，再按需使用底层工具补充细节。请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的时间周期或其他工具组合，基于可用信息提供尽可能全面的分析。"""
