...
async def technical_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            tool_bundle = get_agent_tool_bundle("technical", mcp_tools)
            bundle_info = describe_agent_tool_bundle("technical", mcp_tools)
            logger.info(f"Tool bundle for technical: {bundle_info}")
...
请优先使用高层技能 company_profile_skill 和 technical_analysis_skill 做首轮数据采集，再按需使用底层工具补充细节。请使用可用的工具获取实际数据进行分析，而不是基于假设。"""
