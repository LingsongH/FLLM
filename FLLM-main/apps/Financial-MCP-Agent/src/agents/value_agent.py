...
async def value_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            tool_bundle = get_agent_tool_bundle("value", mcp_tools)
            bundle_info = describe_agent_tool_bundle("value", mcp_tools)
            logger.info(f"Tool bundle for value: {bundle_info}")
...
请优先使用高层技能 company_profile_skill 和 valuation_analysis_skill 做首轮数据采集，再按需使用底层工具补充细节。请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的工具或参数组合，基于可用信息提供尽可能全面的分析。请保持回答简洁，避免冗长的描述性文字"""
