...
async def news_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            tool_bundle = get_agent_tool_bundle("news", mcp_tools)
            bundle_info = describe_agent_tool_bundle("news", mcp_tools)
            logger.info(f"Tool bundle for news: {bundle_info}")
...
请优先使用高层技能 news_analysis_skill 完成首轮新闻采集与整合，再按需回退到 crawl_news 等底层工具。请使用可用的工具获取实际新闻数据进行分析，确保情感分析和风险评估的准确性。如果某些新闻无法获取，请基于可用信息提供尽可能全面的分析。"""
